...
This file is the canonical doc-format contract for the `sdp` skill.
...
docs/
  ...
  <domain>/<feature>.html          domain-scoped feature doc
  <domain>/<sub>/<feature>.html    deeper domain nesting is fine
```

The concrete domain layout is a **per-project fact** — read it from the project's
`CLAUDE.md` (→ _Design docs_), not from this file. The names above are
placeholders, not a fixed taxonomy.

Rules:

- **Domain-scoped paths.** Feature docs live under a domain subfolder matching
  the code they document (e.g. a `client/web` domain → `docs/client/web/input-system.html`).
...
- **`docs/doc.css` is a managed mirror — NEVER hand-edit, inline, or duplicate
  it.** Hard rule, no exceptions: the skill owns that file and overwrites it from
  the canonical [doc.css](doc.css). See _Stylesheet versioning_.

---

## Stylesheet versioning

The visual language lives in a single versioned `doc.css`. The skill's
[doc.css](doc.css) is **canonical**; each project's `docs/doc.css` is an installed
**mirror** of it. Style changes flow one way only: reference → mirror.

- **The version is one integer** in a `doc-css-version:` comment on the file's
  header. That single token is the whole protocol — compare integers, no CSS
  parsing.
- **The skill keeps the mirror current.** On any run that writes or modifies a
  doc, the preamble compares versions by reading only each file's header (never
  the whole stylesheet into context) and, when the project's copy is missing or
  older, replaces it with a byte-for-byte filesystem copy of the reference
  (`cp`, not a read-then-rewrite through the model). A project that is somehow
  ahead is left untouched and flagged.
- **Extending the stylesheet.** When a doc genuinely needs a class the current
  `doc.css` lacks (a new layout, an SVG palette colour), add it **to the skill's
  reference doc.css** and **bump `doc-css-version` by one**. Keep additions
  layout-structural and reuse existing primitives (`.box`, the `box-*` / `arr-*`
  colour variants) before adding cosmetics.
- **Never hand-edit a project's `docs/doc.css`.** It is overwritten from the
  reference whenever it falls behind, so a local edit is drift that silently
...
- All diagrams are built from the same `.box` primitive (below). What changes
  is the **layout** wrapping them. Pick the layout that matches the _shape of
  the flow_ — don't force every feature into the layered stack.
- **Favor a sequence diagram (`.seq-diagram`).** When a flow has any ordering or
  request/response character (a call and its reply, a handshake, a round-trip),
  reach for one first — the time axis is the dimension a box layout silently
  drops. Fall back to a box layout only when there's no ordering to show (see the
  table below and Layout 5).
...
| ------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pipeline through stages         | **Layered stack** (`.canvas`)    | Data passes top→bottom through named layers (e.g. input → schema → state → system) with no meaningful ordering to show. The box-layout default.                                                                                                    |
...
| State chain that loops back     | **Lifecycle** (`.lifecycle`)     | States advance A→B→C then _recycle_ to the start — load/unload, allocate→use→reclaim, pooled resources. The recycle edge is the point; a plain chain can't draw it without duplicating a state.                                                    |
| Ordered exchange between actors | **Sequence** (`.seq-diagram`)    | **The favored layout.** N actors as lifelines, messages as arrows in order, including a feedback edge that returns to an earlier actor. Reach for it whenever ordering / round-trips / a call-and-reply are present — the time axis is what a box layout can't show. |
| Intrinsically geometric         | **Spatial SVG** (`.svg-diagram`) | The relationship _is_ geometry — grid-cell ownership, adjacency, coordinate frames. No arrangement of boxes can show it; only a true drawing can. The rarest layout — reach for it only when a box diagram would lie.                              |
...
1. Add the layout's classes to the skill's **reference** doc.css and bump
   `doc-css-version` (follow the existing naming: `kebab-case`, layout-structural
   only — reuse `.box` and the `box-*` / `arr-*` colour variants, don't add
   cosmetics). Never edit a project's `docs/doc.css`; the preamble propagates the
   new version there.
2. Add a row to the layout table above and a short `#### Layout N` example here,
   so the next run can find and reuse it.
3. Mention the addition in your hand-off so the user knows the reference doc.css
   changed and projects will pick it up on their next run.
...
participant; drawing a node implies it takes part. Something that is _not_ part
...
<div class="fanout">
    <div class="fanout-source">
        <div class="box box-orange">
            <div class="bname">EVENT_NAME</div>
            <div class="bdetail">payload shape</div>
        </div>
    </div>
...
<div class="lifecycle">
    <div class="hflow">
        <div class="box box-violet"><div class="bname">Unloaded</div></div>
...
The favored layout. For an ordered exchange between actors, an inline
`<svg class="seq-diagram">` draws lifelines (vertical) with time running downward
and messages as horizontal arrows. Reach for it whenever _when_ matters —
ordering, a round-trip, or a feedback edge that returns to an earlier actor
(which a box layout cannot show); prefer it over a box layout for any flow with a
call-and-reply or handshake character.
...
For a relationship that _is_ geometry, an inline `<svg class="svg-diagram">` is
...
- If the existing classes don't cover the shape, extend the `.svg-diagram`
  palette in the skill's **reference** doc.css and bump `doc-css-version`
  (reuse-first, same as any layout addition) — don't inline a one-off colour and
  don't edit a project's `docs/doc.css`.
...
A `<p>` _under_ any diagram for a sentence or two of orientation is fine.
...
<table>
    ...
    <tr>
        <td><code>path/to/file.ts</code></td>
        <td>One-line role.</td>
    </tr>
    …
</table>
...
A doc has two audiences, and the layout separates them:

- **The visible structure is for humans.** The problem statement, architecture
  diagram, key-rules tables, and file map give a person the shape of the feature
  at a glance — what it does and how the pieces fit. Keep this layer clean and
  structural; detail a human skimming doesn't need does not belong here.
- **Collapsible `<details>` blocks are for AI agents.** They hold the detail an
  agent needs to act on the feature — to fix it, reimplement it, or extend the
  design — kept out of the human view until an agent opens it. **Detail goes
  here:** push it down into a collapsible rather than spreading it through the
  prose or the diagram.

`<details data-agent="…">` blocks are **always collapsed by default** (no `open`
attribute). Even as the home for detail, a block is not padding — reach for one
only when it genuinely helps an agent and the alternatives below fall short.

### Prefer a source reference over inline detail

Once code exists, the code _is_ the detail. Before writing a block, ask whether
...
**transient scaffolding**: acceptable as a last resort _while the code does not
exist yet_, to carry intent to whoever implements it — but remove it once the
...
Litmus test before keeping a block on a built feature: _would a competent agent
who has read the code and file map still learn something from this?_ If no,
delete it.
...
- **Inline _cosmetic_ CSS or `<style>` blocks.** Colours, borders, fonts,
  backgrounds, spacing — all live in `doc.css`. The only exception is the diagram
  layout hints the primitives require; see _Architecture_ for that boundary.
...
- [ ] `<link rel="stylesheet">` to `doc.css` present via a depth-adjusted
      relative path that reaches `docs/doc.css`; no inline _cosmetic_ CSS (layout
      hints excepted — see _Architecture_).
- [ ] Header has feature name and one-line purpose; no date, no status line.
- [ ] Architecture has a diagram in a layout that fits the flow — a sequence
      diagram where there's ordering / call-and-reply (favored), else layered
      stack / horizontal / fan-out / lifecycle / spatial — or a strong
...
- [ ] Read fresh, the doc accurately predicts the code (or, for a not-yet-built
      feature: predicts what the code _will_ be).
...
- Inline `<style>` blocks, or inline _cosmetic_ styles (colour/border/font/
  background) not sourced from `doc.css` (layout hints excepted — see
  _Architecture_).
...
_meaning_ rather than its form (e.g. a "future" section that might be real
intent, not stray roadmap content). That's a design question — ask before
touching it (see the skill's _Asking_ section).
